  ...
  <div class="mt-[20px]">
      ...
      <div class="w-[65%] mt-[20px] rounded-md overflow-hidden">
        ...
        <div class="mt-[20px] py-[15px] px-[20px] w-full flex flex-col rounded-md bg-white">
          <div   v-for="(sanpham, index) in products"
                      :key="index" class="flex w-full items-start gap-[10px] mb-[30px]">
            <div class="flex items-center gap-[20px]">
              <input :id="sanpham.id"
                            :value="sanpham.id"
                            v-model="selectedProducts" type="checkbox">
              <div class="flex flex-col mt-[20px] w-[102px] h-[102px] items-center justify-center">
                <img  src="https://media.sellycdn.net/files/sm_2023_03_17_04_32_55_0700_ArddHLwscN.jpg" class="mt-1 rounded-md object-cover" alt="">
                <span class="mt-[10px] text-[16px] text-[#6c757d]">SL : {{ getCartItemQuantity(sanpham.id) }}</span>
              </div>
            </div>
            <div class="ml-[18px] flex flex-col w-full gap-[3px]">
              <h2 class="text-[16px] font-bold uppercase text-[#384059]">{{  sanpham.tenSanPham }}</h2>
              <span class="text-[14px] text-[#6C757D]">Phân loại : Trắng</span>
           
              <span class="text-left text-[#cc3366] text-[16px] font-[400]">Tổng giá :     {{
                            getPrice(
              ...
              <div class="flex justify-between items-center">
                <p @click="handleUpdateProduct(sanpham.id , getCartItemQuantity(sanpham.id) ,sanpham.giaBan)" class="text-[16px] mt-0 font-[600] text-[#3478f6]">
                  Sửa
                </p>
                <button class="flex items-center text-[#3478f6]" @click="handleModalDelete(sanpham.id)">
                  <svg
            ...
            <div v-if="isShowModelCart == sanpham.id" class="ModalUpdateCart z-50 fixed w-[512px] py-[20px] px-[10px] h-[630px] bg-white top-[50%] rounded-2xl shadow-2xl left-[50%] translate-x-[-50%] translate-y-[-50%]">
    <div class="float-right">
      <button @click="handleCloseModalData" class="text-[24px] "><font-awesome-icon :icon="['fas', 'circle-xmark']" /></button>
    </div>
    <div class="px-[35px] mt-[40px]">
      <div class="flex gap-[20px]">
        ...
        <div>
          <h1 class="text-[18px] uppercase text-[#384059] font-bold">{{sanpham.tenSanPham}}</h1>
          <div class="flex gap-[20px]">
      ...
      <div class="mt-[20x] py-[20px] text-[16px] leading-[25px] gap-[15px] flex items-center">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" class="flex-shrink-0"><path d="M12 23C18.0751 23 23 18.0751 23 12C23 5.92487 18.0751 1 12 1C5.92487 1 1 5.92487 1 12C1 18.0751 5.92487 23 12 23Z" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path d="M12 11V17" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path d="M12 8C12.5523 8 13 7.55228 13 7C13 6.44772 12.5523 6 12 6C11.4477 6 11 6.44772 11 7C11 7.55228 11.4477 8 12 8Z" fill="currentColor"></path></svg>
              <p>  Sản phẩm đang được khuyến mãi với nhiều ưu đãi đặc biệt quý khách không nên bỏ qua nhé  </p>
              </div>
      <div class="flex justify-between items-center mt-[20px]">
        <h2 class="font-bold text-[18px] text-[#384059]">Số lượng</h2>
        <div class="px-[10px] flex justify-between items-center py-[8px] rounded-xl border-coolGray-500 border-[1px]">
          <button @click="decrement(sanpham.id)" class="w-[20px] h-[20px] flex items-center font-medium justify-center rounded-[50%] bg-black text-white">-</button>
          <input class="w-[100px] text-[16px] text-[#384059] font-bold flex pl-[40px] border-none outline-none" type="number" :value="soLuongUpdate">
          <button @click="increment(sanpham.id)" class="w-[20px] h-[20px] flex items-center font-medium justify-center rounded-[50%] bg-black text-white">+</button>
        </div>
      </div>
      <div class="flex justify-between items-center mt-[40px]">
        <h2 class="font-bold text-[18px] text-[#384059]">Giá</h2>
        <div class="px-[10px] py-[8px] w-[165px] rounded-xl border-[1px] border-coolGray-500">
          <p class="float-right font-medium">{{  giaBan}}</p>
        </div>
      </div>
      <div class="flex justify-between items-center mt-[40px]">
        <h2 class="font-bold text-[18px] text-[#384059]">Tổng giá </h2>
          <p class="float-right text-[#23c6c8] text-[18px] font-medium">{{  getPriceUpdate()
                            }}</p>
      </div>
      <div class="w-full mt-[40px] flex items-center justify-center">
     <div>
      <button @click="handleUpdateNewProductCart(sanpham.id)" class="w-[335px] py-[12px] rounded-xl px-[20px] flex justify-center text-[18px] items-center bg-[#cc3366] text-white font-bold">Cập Nhật</button>
        <button @click="handleModalDelete(sanpham.id)" class="w-[335px] mt-[20px] py-[12px] rounded-xl px-[20px] flex justify-center text-[18px] items-center bg-white border-[1px] border-[#cc3366]  text-[#cc3366] font-bold">Xóa Sản Phẩm</button>
     </div>
      </div>
    </div>
            </div>
            <div v-if="isShowModalOpacity==true" @click="isShowModelCart=false" class="fixed top-0 lef-0 right-0 w-full h-full bg-black opacity-25 z-20"></div>
            <div v-if="isshowModalDelete" class="ModalUpdateCart z-50 fixed overflow-hidden bg-white top-[50%] translate-x-[-50%] rounded-2xl shadow-2xl left-[50%] translate-y-[-50%]">
              <div class="float-right pr-[20px] pt-[10px]">
                <button @click="handleCloseModalData" class="text-[24px] "><font-awesome-icon :icon="['fas', 'circle-xmark']" /></button>
              </div>
              <div class="flex items-center justify-center mt-[60px]">
                <svg width="91" height="90" viewBox="0 0 91 90" fill="none"><circle opacity="0.1" cx="45.5" cy="45" r="45" fill="#CC3366"></circle><circle cx="45.5" cy="45" r="30" fill="#CC3366"></circle><path d="M50.5549 53.603L50.2489 54.857C49.2109 55.266 45.6149 56.982 43.5409 55.156C42.9229 54.613 42.6139 53.923 42.6139 53.087C42.6139 51.52 43.1299 50.154 44.0559 46.874C44.2189 46.255 44.4189 45.45 44.4189 44.812C44.4189 43.712 44.0019 43.419 42.8689 43.419C42.3159 43.419 41.7039 43.616 41.1499 43.823L41.4569 42.569C42.6919 42.067 44.2429 41.455 45.5719 41.455C47.5649 41.455 49.0299 42.449 49.0299 44.339C49.0299 44.884 48.9359 45.838 48.7379 46.498L47.5919 50.552C47.3559 51.372 46.9259 53.178 47.5899 53.714C48.2449 54.243 49.7919 53.963 50.5549 53.603Z" fill="white"></path><path d="M48.5 39C49.8807 39 51 37.8807 51 36.5C51 35.1193 49.8807 34 48.5 34C47.1193 34 46 35.1193 46 36.5C46 37.8807 47.1193 39 48.5 39Z" fill="white"></path></svg>
              </div>
              <div class="flex flex-col mt-[40px] text-center justify-center  px-[20px]">
                <h1 class="text-[22px] font-bold">Bỏ sản phẩm</h1>
                <p class="text-[18px] font-[600]">Bạn có chắc muốn bỏ sản phẩm này chứ</p>
              </div>
              <div class="flex w-full mt-[30px]">
                <button @click="hadleReturnModelCart" class="hover:bg-[#ad2b57] w-[50%] py-[18px] px-[18px] text-[18px] text-white bg-[#CC3366] border-r-[1px]">Không</button>
                <button class="hover:bg-[#ad2b57] flex-1 text-[18px] text-white bg-[#CC3366] py-[18px] px-[18px]" @click="handleDelete">Tiếp Tục</button>
              </div>
            </div>
            </div>
      ...
      <div class="flex-1 bg-white mt-[20px] rounded-md px-[20px] py-[15px]">
        ...
        <button @click="checkOut" :class="selectedProducts.length > 0 ? 'mt-[40px] w-full py-[15px] text-white bg-[#cc3366] rounded-xl' : 'mt-[40px] w-full py-[15px] text-white bg-[#9b9fac] rounded-xl'">Tạo Đơn</button>
         </div>
      </div>
